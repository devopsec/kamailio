 ...
 */
static void dispatcher_rpc_test_ds_select(rpc_t *rpc, void *ctx)
{
	int i;
	int dst_set;
	int dst_alg;
	...
	// ds_set_t *dst_nodes = NULL;
	sr_xavp_t *ds_xavp = NULL;
	sr_xavp_t *xavp_pit = NULL;
	...
	// dst_nodes = ds_list_lookup(dst_set);
	// if(dst_nodes == NULL) {
	// 	rpc->fault(ctx, 404, "destination set not found");
	// 	return;
	// }
	...
	if (ds_select_dst_limit(&faked_msg, dst_set, dst_alg,
		(unsigned int)dst_limit, DS_SETOP_XAVP) < 0) {
	...
	// return results
	// TODO: handle no failure mode
	// TODO: handle grabbing results from parallel alg
	if(rpc->add(ctx, "{", &rpc_handle) < 0) {
		rpc->fault(ctx, 500, "Internal error root reply");
		goto end1;
	}
	i = 0;
	do {
		...
		xavp_pit = ds_xavp->val.v.xavp;
		while(xavp_pit != NULL) {
			if (strncmp(xavp_pit->name.s, ds_xavp_dst_addr.s,
				ds_xavp_dst_addr.len) == 0) {
				snprintf(int_buf, 12, "%d", i);
				if(rpc->struct_add(rpc_handle, "S", int_buf,
					&xavp_pit->val.v.s) < 0) {
					rpc->fault(ctx, 500, "Internal error main structure");
					goto end1;
				}
...
static const char *dispatcher_rpc_test_ds_select_doc[2] = {
	"Summary:\tShow the destination results if ds_select() was ran\n"
	"Usage:\tdispatcher.test_ds_select __dst_set__ __dst_alg__ [__dst_limit__] "
...
rpc_export_t dispatcher_rpc_cmds[] = {
	...
	{"dispatcher.test_ds_select", dispatcher_rpc_test_ds_select,
		dispatcher_rpc_test_ds_select_doc, RPC_RET_ARRAY},
	{0, 0, 0, 0}
};
/* clang-format on */
